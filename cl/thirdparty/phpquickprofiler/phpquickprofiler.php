...
class PhpQuickProfiler {
	...
	public function __construct($startTime, $config = null) {
		is_null($config) and $config = array(
			'open'	=> false
		);
	...
	{
		D5_Arr::set($this->config, $key, $value);
	}
	...
	function attemptToExplainQuery($query) {
		if (substr($query['sql'],0,6) == 'SELECT')
		{
			$rs = false;
			try {
				$sql = 'SELECT EXPLAIN '.html_entity_decode($query['sql'], ENT_QUOTES);
				$rs = DB::instance($query['dbname'])->query($sql)->fetchAll(D5_DB_Resultset::FETCH_ASSOC);
			}
			catch(Exception $e)
			{}
	...
	public function gatherSpeedData() {
		$speedTotals = array();
		$speedTotals['total'] = $this->getReadableTime((self::getMicroTime() - $this->startTime)*1000);
	...
	public function display($db = '') {
		...
		if ($this->output['logs']['errorCount'] > 0) {
			$this->config['open'] = true;
			$this->config['tall'] = true;
		}
